...
template <class T>
Doub zbrent(T &func, const Doub x1, const Doub x2, const Doub tol, int &NIter)			// Changed by Dmunoz
// Doub zbrent(T& func, const Doub x1, const Doub x2, const Doub tol)					// Initialy
{
	const Int ITMAX=1000;			// Changed by Dmunoz, ITMAX=100; was initially
	const Doub EPS=numeric_limits<Doub>::epsilon();
	Doub a=x1,b=x2,c=x2,d,e,fa=func(a),fb=func(b),fc,p,q,r,s,tol1,xm;
	if ((fa > 0.0 && fb > 0.0) || (fa < 0.0 && fb < 0.0))
	// throw("Root must be bracketed in zbrent");		// Initially
	{
		NIter = -1; return 0.0;					// Changed by Dmunoz
	}
	fc=fb;
	for (Int iter=0;iter<ITMAX;iter++) {
		...
		xm=0.5*(c-b);
		if (abs(xm) <= tol1 || fb == 0.0) { NIter = iter;  return b; }		// Changed by Dmunoz
		// return b;	// Originally
	...
	// throw("Maximum number of iterations exceeded in zbrent");		// Initially
		NIter = -2; return 0.0;											// Changed by Dmunoz
}
template <class T>
