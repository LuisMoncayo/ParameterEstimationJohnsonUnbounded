...
public:
	...
	//void sort(const int &l, const int &r, long *mark);	//quick sort 
	~NRvector();
};
...
// Useful operations with NRVector
template <class T>
T NRdotprod(const int &NSum, const NRvector<T> &V1, const int &IniV1, const NRvector<T> &V2, const int &IniV2)
{// dot product of two NRvector, assumed overloaded for '+',  '<' , '*' and has 0 defined
	int n1 = V1.size(), n2 = V2.size(), i = 0, i1 = IniV1, i2 = IniV2;
	T Sum = 0;
	while ((i < NSum) && (i1 < n1) && (i2 < n2))
	{
		Sum += V1[i1] * V2[i2];		i += 1;			i1 += 1;			i2 += 1;
	}
	return Sum;
}

template <class T>
void NRsumconst(const int &NSum, NRvector<T> &V, const T &c, const int &IniV)
{// Sum c to NSum elements of V starting from IniV
	int n1 = V.size(), i = 0, i1 = IniV;
	while ((i < NSum) && (i1 < n1))
	{
		V[i1] += c;					i += 1;			i1 += 1;
	}
...
template <class T>
void NRmultconst(const int &NSum, NRvector<T> &V, const T &c, const int &IniV)
{// Multiplies c to NSum elements of V starting from IniV
	int n1 = V.size(), i = 0, i1 = IniV;
	while ((i < NSum) && (i1 < n1))
	{
		V[i1] *= c;					i += 1;			i1 += 1;
