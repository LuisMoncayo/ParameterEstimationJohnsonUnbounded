...
#include "LBFGSpp/LineSearchMoreThuente1.h"     // Changed by DMUnoz
...
private:
    ...
    static Scalar max_step_size(const Vector& x0, const Vector& drt, const Vector& lb, const Vector& ub)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            if (drt[i] > Scalar(0))
            {
                step = MIN(step, (ub[i] - x0[i]) / drt[i]);  // std::min(step, (ub[i] - x0[i]) / drt[i]); // DMunoz change
            }
            else if (drt[i] < Scalar(0))
            {
                step = MIN(step, (lb[i] - x0[i]) / drt[i]); // std::min(step, (lb[i] - x0[i]) / drt[i]);    // DMunoz change
            }
        }
...
public:
    ...
    ///        algorithm
    ///
    int CodeE = 0;  // DMunoz change to register an exception code, 0 is no exception occured
    ...
    inline int minimize(Foo& f, Vector& x, Scalar& fx, const Vector& lb, const Vector& ub)
    {
        ...
        {
            CodeE = -1;
            return -1;  // throw std::invalid_argument("'lb' and 'ub' must have the same size as 'x'");     // DMunoz change
        }
        ...
        for (;;)
        {
            ...
            step_max = MIN(m_param.max_step, step_max); // std::min(m_param.max_step, step_max); //DMunoz change
            Scalar step = Scalar(1); 
            step = MIN(step, step_max); // std::min(step, step_max);    //DMunoz change
            // if (k == 4) { cout << "Error detected";}
            LineSearch<Scalar>::LineSearch(f, m_param, m_xp, m_drt, step_max, step, fx, m_grad, dg, x, CodeE);  // DMunoz change
            if (CodeE < 0){ 
                CodeE = -5 + CodeE;    return k;   }  // DMunoz change
            ...
            if (fpast > 0)
            {
                ...
                if (k >= fpast && abs(fxd - fx) <= m_param.delta * MAX(MAX(abs(fx), abs(fxd)), Scalar(1))) // std::max(std::max(abs(fx), abs(fxd)), Scalar(1))) //DMunoz change
                    return k;
            ...
            {// m_param.max_iterations == 0 means no max is proposed
                CodeE = -2;     // DMunoz change
                return k; 
            }
